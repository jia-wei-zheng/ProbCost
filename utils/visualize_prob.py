...
def graphviz_visualization(net, image_format="png", initial_marking=None, final_marking=None, decorations=None,
    ...
    for t in net.transitions:
        if t.label is not None:
            if t in decorations and "label" in decorations[t] and "color" in decorations[t]:
                viz.node(str(id(t)), decorations[t]["label"], style='filled', fillcolor="green",
                         border='1')
            else:
                # viz.node(str(id(t)), str(t.label) + "<BR /><FONT POINT-SIZE='10'>" + str(decorations[t]["prob"]) + "</FONT>")
                viz.node(str(id(t)), str(t.label), xlabel = str(decorations[t]["prob"]), fontsize='20')
    ...
    # places
    viz.attr('node', shape='circle', fixedsize='true', width='0.65')
