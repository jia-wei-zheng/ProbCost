...
def apply_sync_prod(sync_prod, initial_marking, final_marking, cost_function, normal_cost, skip, ret_tuple_as_trans_desc=False,
    ...
    cost_function: :class:`dict` cost function transfered from probability mapping transitions to the synchronous product net
    # normal_cost: class: `dict` normal cost function mapping transitions to the synchronous product net, for comparing the most optimal 
    ...
    and **traversed_arcs**
    """
    return __search(sync_prod, initial_marking, final_marking, cost_function, normal_cost, skip,
                    ret_tuple_as_trans_desc=ret_tuple_as_trans_desc, max_align_time_trace=max_align_time_trace)


def __search(sync_net, ini, fin, cost_function, normal_cost, skip, ret_tuple_as_trans_desc=False,
             max_align_time_trace=sys.maxsize):
    start_time = time.time()
    ...
    ini_state = utils.SearchTuple(0 + h, 0, 0, h, ini, None, None, x, True)
    open_set = [ini_state]
    ...
    while not len(open_set) == 0:
        ...
        while not curr.trust:
            ...
            tp = utils.SearchTuple(curr.g + h, curr.g, curr.n_cost, h, curr.m, curr.p, curr.t, x, True)
        ...
        for t, cost in trans_to_visit_with_cost:
            ...
            if new_marking in closed:
                continue
            g = curr.g + cost
            n_cost = curr.n_cost + normal_cost[t]
            ...
            tp = utils.SearchTuple(new_f, g, n_cost, h, new_marking, curr, t, x, trustable)
